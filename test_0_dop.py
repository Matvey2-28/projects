...
def cicloid():
    ...
    fig, ax = plt.subplots()
    cicloid, = plt.plot([], [], color='r', label='Ball')
    ...
    def animate(i):
        ...
        coords[i] = cicloid_move(R=1, vx0=0.01, time=i)
        cicloid.set_data([coords[i][0]], [coords[i][1]])
        cicloid_line.set_data(coords[:i, 0], coords[:i, 1])
        return cicloid, cicloid_line
